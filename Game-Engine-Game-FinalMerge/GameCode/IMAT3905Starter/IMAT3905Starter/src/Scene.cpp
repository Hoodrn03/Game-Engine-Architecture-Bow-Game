...
Scene::Scene(std::string filename, ModelManager* theModelManager, IEngineCore* engineCore) : m_theModelManager(theModelManager)
{
	...
	m_CollisionDetector.m_SetEnemyObject(getNPCCharacter());
...
bool Scene::loadLevelJSON(std::string levelJSONFile)
{
	...
	for (int i = 0; i < (int)gameObjects.size(); i++)
	{
		...
		if (i == m_playerIndex)
		{
			v_gameObjects.push_back(new PlayerCharacter(model, position, orientation, 55000.0f, 55.5f));
		}
		else if (i == m_enemyIndex)
		{
			v_gameObjects.push_back(new NPC_Character(model, position, orientation, 55000.0f, 55.5f));
		}
		else
		{
